...
function ScoreBar({ label, score, max = 10, accent = '#00c8e8' }: { label: string; score: number; max?: number; accent?: string }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{
        background: 'rgba(255,255,255,0.06)', borderRadius: 3, height: 6, overflow: 'hidden',
      ...
      {[1, 2, 3, 4, 5].map(star => (
        <span key={star}
          ...
          style={{
            ...
            color: star <= value ? '#ffab00' : '#3a3545',
...
export default function ToolDetail() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        if (data?.tags?.length) {
          promises.push(
            supabase.from('tools').select('id, name, url, description, tags, stars, overall_score, deploy_type')
        ...
        if (ratingsData?.length) {
          setAvgRating(Math.round(ratingsData.reduce((s: number, r: any) => s + r.rating, 0) / ratingsData.length))
        ...
        const ri = user ? 3 : 1
        if (results[ri]?.data) setRelatedTools(results[ri].data)
  ...
  async function handleRate(rating: number) {
    ...
    try {
      await supabase.from('user_ratings').upsert(
        { tool_id: toolId, user_id: user.id, rating, updated_at: new Date().toISOString() },
        { onConflict: 'user_id,tool_id' }
      )
      setUserRating(rating)
      const { data: rd } = await supabase.from('user_ratings').select('rating').eq('tool_id', toolId)
      if (rd?.length) {
        setAvgRating(Math.round(rd.reduce((s: number, r: any) => s + r.rating, 0) / rd.length))
  ...
  return (
    <div className="container fade-in" style={{ maxWidth: 960 }}>
      {/* 面包屑 */}
      <Link href="/" style={{
        color: '#6e6878', fontSize: 13, fontWeight: 500, textDecoration: 'none',
        display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 28,
        transition: 'color 0.15s',
      }}
        onMouseEnter={e => (e.currentTarget.style.color = '#a09baa')}
        onMouseLeave={e => (e.currentTarget.style.color = '#6e6878')}
      >
        ← 返回首页
      </Link>
      ...
      <div className="card" style={{
        marginBottom: 32, padding: '36px 32px',
        background: 'linear-gradient(135deg, rgba(0,200,232,0.04) 0%, rgba(0,230,118,0.02) 100%)',
        border: '1px solid rgba(255,255,255,0.08)',
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 24 }}>
          <div style={{ flex: 1, minWidth: 260 }}>
            <h1 style={{ fontSize: '2rem', fontWeight: 700, lineHeight: 1.2, marginBottom: 8, letterSpacing: '-0.02em' }}>
              {tool.name}
            </h1>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 16 }}>
              {tool.url && <a href={tool.url} target="_blank" rel="noopener noreferrer" className="btn">🔗 访问工具</a>}
              {tool.source_url && <a href={tool.source_url} target="_blank" rel="noopener noreferrer" className="btn btn-outline">
                📰 {tool.source || '来源'}
              </a>}
          ...
          <div style={{
            textAlign: 'center',
            background: `linear-gradient(135deg, ${score >= 8 ? 'rgba(0,230,118,0.1)' : 'rgba(255,171,0,0.1)'}, transparent)`,
            border: `1px solid ${score >= 8 ? 'rgba(0,230,118,0.2)' : 'rgba(255,171,0,0.2)'}`,
            borderRadius: 20, padding: '20px 32px', minWidth: 110,
          }}>
            <div style={{
              fontSize: 56, fontWeight: 800, lineHeight: 1,
              background: `linear-gradient(135deg, ${score >= 8 ? '#00e676' : '#ffab00'}, ${score >= 8 ? '#00c8e8' : '#ff6d00'})`,
              WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
            }}>{score}</div>
            <div style={{ color: '#6e6878', fontSize: 11, fontWeight: 500, marginTop: 4, letterSpacing: '0.02em' }}>
              综合评分 / 10
            </div>
            {avgRating > 0 && (
              <div style={{ color: '#ffab00', fontSize: 12, fontWeight: 600, marginTop: 8 }}>
                ⭐ {avgRating} ({ratingCount} 人)
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 24 }}>
          {(tool.tags || []).map((tag, i) => (
            <Link key={i} href={`/search?q=${encodeURIComponent(tag)}`} className="tag"
              style={{ textDecoration: 'none', border: '1px solid rgba(0,230,118,0.15)' }}>
              {tag}
            </Link>
          ))}
          {tool.deploy_type && (
            <span style={{ padding: '4px 12px', background: 'rgba(0,200,232,0.08)', color: '#00c8e8', borderRadius: 8, fontSize: 12, fontWeight: 500 }}>
              {deployIcons[tool.deploy_type] || '📦'} {tool.deploy_type}
            </span>
          )}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            <div className="card" style={{ border: '1px solid rgba(255,171,0,0.12)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                {(tool.suitable_for || []).map((s, i) => (
                  <span key={i} style={{
                    padding: '8px 16px', background: 'rgba(255,171,0,0.06)', border: '1px solid rgba(255,171,0,0.15)',
                    borderRadius: 20, color: '#ffab00', fontSize: 13, fontWeight: 500,
                  }}>{s}</span>
                ))}
              </div>
              {/* 在线预览 */}
          ...
          {verdictBlocks.length > 0 && (
            <div className="card" style={{
              ...
              background: 'linear-gradient(180deg, rgba(0,200,232,0.03) 0%, transparent 200px)',
            }}>
              <h2 style={{ fontSize: '1.05rem', fontWeight: 700, color: '#f0edf5', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{
                  ...
                  background: 'rgba(0,200,232,0.12)', textAlign: 'center', lineHeight: '32px', fontSize: 16,
                }}>🔬</span> 评测详情
              </h2>
              {verdictBlocks.map((block, i) => {
                const isProcess = block.title.includes('流程')
                const isResult = block.title.includes('结果')
                const isTestImg = block.title.includes('测试图')
                ...
                return (
                  <div key={i} style={{ marginBottom: i < verdictBlocks.length - 1 ? 28 : 0 }}>
                    <h3 style={{
                      fontSize: '0.95rem', fontWeight: 600, marginBottom: 14, paddingBottom: 10,
                      borderBottom: '1px solid rgba(255,255,255,0.05)',
                      color: isProcess ? '#ffab00' : isResult ? '#00e676' : isTestImg ? '#ff9100' : '#a09baa',
                    }}>
                          ...
                          return (
                            <div key={si} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                              <div style={{
                                ...
                                background: 'rgba(255,171,0,0.1)', color: '#ffab00',
                                display: 'flex', alignItems: 'center', justifyContent: 'center',
                                fontSize: 13, fontWeight: 700,
                              }}>{match?.[1] || si + 1}</div>
                              <span style={{ color: '#ccc', lineHeight: 1.7, fontSize: 14, paddingTop: 4 }}>
                                {match?.[2] || step}
                              </span>
                            </div>
                          )
          ...
          {tool.tested_at && (
            <div className="card" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
              <h2 style={{ fontSize: '1.05rem', fontWeight: 700, color: '#f0edf5', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{
                  display: 'inline-block', width: 32, height: 32, borderRadius: 8,
                  background: 'rgba(0,230,118,0.1)', textAlign: 'center', lineHeight: '32px', fontSize: 16,
                }}>🖼️</span> 测试截图
              </h2>
              <p style={{ color: '#6e6878', fontSize: 12, marginBottom: 20 }}>
                实测过程中生成的全部截图和切图产物
              </p>
              <div style={{
                ...
                {evalImages.map((img, i) => (
                  <a key={i} href={img.src} target="_blank" rel="noopener noreferrer"
                    style={{
                      display: 'block', background: '#16161f', borderRadius: 10,
          ...
          {(tool.pros?.length > 0 || tool.cons?.length > 0) && (
            <div style={{
              display: 'grid',
              gridTemplateColumns: `${(tool.pros?.length && tool.cons?.length) ? '1fr 1fr' : '1fr'}`,
              gap: 16,
            }}>
              {tool.pros?.length > 0 && (
                <div className="card" style={{
                  border: '1px solid rgba(0,230,118,0.15)',
                  background: 'linear-gradient(180deg, rgba(0,230,118,0.03) 0%, transparent 60px)',
                }}>
                  <h3 style={{ fontSize: '0.95rem', fontWeight: 600, color: '#00e676', marginBottom: 14 }}>
                    ✅ 实测优点
                  </h3>
                  <ul style={{ paddingLeft: 16, color: '#ccc', fontSize: 13, lineHeight: 2, margin: 0 }}>
              ...
              {tool.cons?.length > 0 && (
                <div className="card" style={{
                  ...
                  background: 'linear-gradient(180deg, rgba(255,82,82,0.02) 0%, transparent 60px)',
                }}>
                  <h3 style={{ fontSize: '0.95rem', fontWeight: 600, color: '#ff5252', marginBottom: 14 }}>
                    ❌ 不足
                  </h3>
                  <ul style={{ paddingLeft: 16, color: '#ccc', fontSize: 13, lineHeight: 2, margin: 0 }}>
          ...
          {/* 用户评分 */}
          <div className="card" style={{ border: '1px solid rgba(255,171,0,0.12)' }}>
            <h3 style={{ fontSize: '0.95rem', fontWeight: 600, color: '#ffab00', marginBottom: 12 }}>
              ⭐ 你的评分
            </h3>
            {avgRating > 0 && (
              <p style={{ color: '#6e6878', fontSize: 12, marginBottom: 10 }}>
                当前平均 {avgRating} 分（{ratingCount} 人评分）
              </p>
            )}
            <StarRating value={userRating} onChange={handleRate} />
            {userRating > 0 && (
              <p style={{ color: '#00e676', fontSize: 12, fontWeight: 500, marginTop: 10 }}>
                ✓ 已提交 {userRating} 星评分
              </p>
            )}
            {!user && (
              <p style={{ color: '#6e6878', fontSize: 12, marginTop: 10 }}>
                <Link href="/auth/login" style={{ color: '#00c8e8', textDecoration: 'underline' }}>登录</Link> 后可参与评分
              </p>
            )}
          </div>
          ...
          <div style={{
            ...
            <a
              ...
              style={{
                padding: '6px 14px', background: 'rgba(29,161,242,0.1)', border: '1px solid rgba(29,161,242,0.2)',
                borderRadius: 6, color: '#1da1f2', fontSize: 12, fontWeight: 500, textDecoration: 'none',
              }}
            >𝕏 Twitter</a>
            <button
              onClick={() => { navigator.clipboard.writeText(window.location.href).catch(() => {}); alert('链接已复制！') }}
              style={{
                padding: '6px 14px', background: 'rgba(0,200,232,0.08)', border: '1px solid rgba(0,200,232,0.15)',
                borderRadius: 6, color: '#00c8e8', fontSize: 12, fontWeight: 500, cursor: 'pointer',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20, position: 'sticky', top: 24 }}>
          ...
          {(tool.ease_score > 0 || tool.useful_score > 0 || tool.hype_score > 0) && (
            <div className="card" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
              <h3 style={{ fontSize: '0.9rem', fontWeight: 600, color: '#f0edf5', marginBottom: 16 }}>
                📊 多维评分
              </h3>
          ...
          {tool.tested_at && (
            <div className="card" style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{
                  width: 40, height: 40, borderRadius: 10, background: 'rgba(0,230,118,0.1)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18,
                }}>🧪</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#00e676', marginBottom: 2 }}>已实测验证</div>
                  <div style={{ color: '#6e6878', fontSize: 11, lineHeight: 1.4 }}>
                    {new Date(tool.tested_at).toLocaleDateString('zh-CN', {
          ...
          {relatedTools.length > 0 && (
            <div className="card" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
              <h3 style={{ fontSize: '0.9rem', fontWeight: 600, color: '#f0edf5', marginBottom: 14 }}>
                🔗 相关工具
              </h3>
                ...
                {relatedTools.map((rt: any) => (
                  <Link key={rt.id} href={`/tools/${rt.name}-${rt.id.slice(0, 8)}`}
                    style={{
                      ...
                      border: '1px solid rgba(255,255,255,0.04)', textDecoration: 'none', color: '#ccc',
                  ...
                  >
                    ...
                    <div style={{ fontSize: 11, color: '#6e6878', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
